...
| 3 | India's telecom rules were updated in 2024, replacing the 2017-era framework | Reviewed; `COMPLIANCE.md` keeps to spectrum, licence, and warranty scope only |
...
- **Confirmed (high):** a cryptography-based networking stack running across **LoRa, packet
  radio, WiFi/Ethernet, serial, I2P** heterogeneous by design, matching Project Mesh's multi-transport
...
- **Recommendation:** treat Reticulum as a **primary design reference and interop target**
  ...
  Project Mesh's sole protocol foundation** given the spec + governance risk. Watch the Rust
  reimplementation (`reticulum-rs`), and revisit if it matures.
...
- **Berty (Oct 2025).** Wesh protocol, BLE, server-less, but the Android app was **unavailable
...
- **Groups:** **switch to MLS (RFC 9420, Jul 2023).** MLS gives async group key establishment with
  FS **and** PCS and **logarithmic** key-update cost via **TreeKEM**, versus Double-Ratchet/sender-key
  group schemes whose update cost grows **~O(N²)**. Project Mesh's original "seal one copy per member"
...
- **Confirmed (high):** a backgrounded iOS app's service UUIDs are moved to a special **"overflow"
  ...
  bitmask**, and each UUID sets one bit. Discoverable **only by a device explicitly scanning for that
...
(Ministry of Communications, WPC Wing), verified by extracting the official gazette PDF text.
...
- **Conditions:** no licence needed for compliant devices, but on a **non-interference,
  non-protected, shared** basis, and equipment must be **type-approved** under applicable
  telecom rules. Legal basis: Indian Telegraph Act 1885 + Indian Wireless Telegraphy Act 1933.
...
## 5. Regulatory landscape: reviewed, kept out of scope here
...
Telecom regulation changes over time and differs by country, and a running legal commentary does
not belong in a technical research report. This review found that India's telecom rules were
updated in 2024, replacing the older 2017-era framework, and that the position on messaging
services more broadly is still developing. None of that detail was carried into the
documentation set: `COMPLIANCE.md` keeps to the scope that matters day to day for engineering
and for users, which is spectrum rules, the licence, and the warranty position.

One design point from this review is worth keeping as a finding, since it comes from the
architecture rather than from any specific rule: Betar has no server, no operator account, and
no central directory, so there is no message-origin record for it to produce and no single
party for a request to attach to. That is a property of how the app is built, not a legal
conclusion, and it should be checked against current regulation before any general release. A
qualified legal review of the current rules is worth commissioning before then, independent of
anything written here.

---

## 6. Traceability and hackability: what the evidence supports

The original goal was "not traceable, not hackable." The verified evidence says plainly that
this is not achievable, and no credible project claims it. What is achievable is
...
- **BLE physical-layer fingerprinting (IEEE S&P 2022, high).** Hardware imperfections,
  **Carrier Frequency Offset** and **I/Q offset**, let an attacker fingerprint a BLE device at
  ...
  variation, TX-power, receiver quality), so it is **feasible but not universally reliable**:
  sometimes trackable, not always and not never.
...
replaced with plain, bounded language and an explicit statement that a determined local RF
adversary may fingerprint/track devices despite MAC randomization. Being upfront about this
limit is itself a security and legal asset (§0, `THREAT-MODEL.md`, `COMPLIANCE.md`).
...
5. **[Fixed] iOS/Android background discovery overstated as impossible.** Corrected; screen-gated
   nuance documented.
6. **[Reviewed] Regulatory landscape was out of date.** Refreshed the review; scope kept to
   spectrum, licence, and warranty in `COMPLIANCE.md` rather than a running legal commentary.
7. **[Open] TOFU without verification is a known break vector** (Bridgefy 2022). Betar already
...
- Regulatory review refreshed; `COMPLIANCE.md` keeps to spectrum, licence, and warranty scope. A
  qualified legal review of the current rules is worth commissioning before general release.
- Remove all "untraceable/unhackable" language; ship plain, bounded claims.
...
- Keep the design's documented limits as a first-class feature.
...
2022), and credible legal analyses (SFLC.in, IFF, SCC Online, gazette notifications), and
cross-checked by 3-vote adversarial verification. The full bibliography is
