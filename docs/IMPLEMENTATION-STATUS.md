...
| Transport trait exposed over UniFFI | `ARCHITECTURE.md` §3, `TRANSPORT.md` | ✅ | `core/src/ffi_transport.rs` | `FfiMeshTransport` callback interface (Kotlin/Swift implements: start/stop/send — Rust calls it) + `FfiTransportHub` (native calls in: on_peer_discovered/on_peer_connected/on_frame/on_peer_lost, logged and drainable). 3 tests using a mock loopback transport, proving the bidirectional FFI plumbing without hardware. Does **not** yet parse frames, hand them to `FfiDurableStore`, or make relay decisions — that's a distinct "mesh engine loop" increment |
...
| UniFFI bindings | `ARCHITECTURE.md` §6 | ✅ | `core/src/ffi.rs`, `core/src/ffi_transport.rs` | identity, Noise handshake (`FfiHandshake`), ratchet session (`FfiSession`), in-memory store (`FfiStore`), standalone durable store (`FfiEncryptedStore`), the two wired together (`FfiDurableStore` — use this one in a real app), envelope pack/unpack, transport callback interface (`FfiMeshTransport`/`FfiTransportHub`) all exported + tested (13 FFI-layer tests: 10 in `ffi.rs`, 3 in `ffi_transport.rs`); Kotlin bindings generated (`core/generated/`, copied to `android/`, 4,972 lines). Not exported yet: MLS, channels, onion routing |
