...
| UniFFI bindings | `ARCHITECTURE.md` §6 | ✅ | `core/src/ffi.rs` | identity, Noise handshake (`FfiHandshake`), ratchet session (`FfiSession`), in-memory store (`FfiStore`), standalone durable store (`FfiEncryptedStore`), the two wired together (`FfiDurableStore` — use this one in a real app), envelope pack/unpack all exported + tested (12 FFI-layer tests); Kotlin bindings generated (`core/generated/`, copied to `android/`, 3,822 lines). Not exported yet: MLS, channels, onion routing, transport callback interfaces |
