...
- **Honest limit:** deliberate **radio jamming** of the 2.4 GHz band (BLE/Wi-Fi) is a physical
  attack no app can fully defeat; the sub-GHz LoRa backbone (Phase 3) is a mitigation because it
  uses a different band, not a cure.
...
- **Honest limit:** against a well-resourced forensic adversary with an unlocked device or a
  device-OS exploit, no application-level measure is a guarantee.

## 3. Adversaries **out of scope** (non-goals)

Stated plainly so no user is misled:
...
| Learn long-term identity from handshake | A1 | Noise XX identity hiding | — |
| Track device by link address | A1 | Rotating randomized identifiers | Fingerprinting by other radio features (partial) |
| Inject / forge messages | A2 | AEAD integrity, signatures | — |
| Replay old messages | A2 | Content-ID + seen-set | — |
...
| Recover past msgs after key leak | — | Forward secrecy (ratchet) | — |
| Read future msgs after key leak | — | Post-compromise security (ratchet) | Window before self-heal |
...
- **No central operator** means there is no single party who can be legally compelled to hand
  over data, because no such data or party exists. This is a security property *and* a legal one
  (see `LEGAL.md`).
