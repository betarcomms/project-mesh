...
//!
//! This is the in-memory index — fast dedup/eviction decisions, no disk I/O on the hot path.
//! [`crate::durable::DurableStore`] pairs it with [`crate::persistence::EncryptedStore`] for
//! durability across restarts; `accept`/`purge_expired` return the IDs that left the store so
//! that wiring can mirror removals to disk without re-deriving eviction logic itself.
...
impl Store {
    ...
    ///
    /// Returns the outcome plus the ID of any envelope evicted to make room for this one, so a
    /// caller backing this store with durable storage (see [`crate::durable::DurableStore`])
    /// can mirror the removal without re-deriving eviction logic.
    pub fn accept(&mut self, envelope: Envelope, now: u64) -> (Accept, Option<EnvelopeId>) {
        if self.entries.contains_key(&envelope.id) {
            return (Accept::Duplicate, None);
        }
        if envelope.is_expired(now) {
            return (Accept::Expired, None);
        }
        if envelope.ttl_hops == 0 {
            return (Accept::TtlExhausted, None);
        }
            ...
            match self.worst_entry_id() {
                Some(worst_id) => {
                    ...
                    if envelope.priority >= worst_priority {
                        ...
                        return (Accept::StoreFullLowerPriority, None);
                    }
                    self.entries.remove(&worst_id);
                    evicted = Some(worst_id);
                }
        ...
        if self.entries.len() >= self.capacity {
            return (Accept::StoreFullLowerPriority, evicted);
        }
    ...
    /// Remove all expired envelopes; returns the IDs that were purged (so a durable-backed
    /// caller can mirror the removal to disk).
    pub fn purge_expired(&mut self, now: u64) -> Vec<EnvelopeId> {
        let expired_ids: Vec<EnvelopeId> = self
            .entries
            .iter()
            .filter(|(_, e)| e.envelope.is_expired(now))
            .map(|(id, _)| *id)
            .collect();
...
mod tests {
    ...
    fn accepts_new_and_rejects_duplicate() {
        ...
        assert_eq!(store.accept(e.clone(), 0), (Accept::New, None));
        assert_eq!(store.accept(e, 0), (Accept::Duplicate, None));
    ...
    fn rejects_expired_and_ttl_exhausted() {
        let mut store = Store::new(10);
        assert_eq!(
            store.accept(env(Priority::Normal, 5, 100, 1), 200),
            (Accept::Expired, None)
        );
        assert_eq!(
            store.accept(env(Priority::Normal, 0, 1000, 2), 0),
            (Accept::TtlExhausted, None)
        );
    ...
    fn evicts_lowest_priority_when_full() {
        ...
        let low = env(Priority::Low, 5, 1000, 1);
        let low_id = low.id;
        assert_eq!(store.accept(low, 0), (Accept::New, None));
        assert_eq!(store.accept(env(Priority::Normal, 5, 1000, 2), 0), (Accept::New, None));
        // Store full; SOS must evict the Low-priority entry, not be refused.
        let sos = env(Priority::Sos, 5, 1000, 3);
        let (outcome, evicted) = store.accept(sos.clone(), 0);
        assert_eq!(outcome, Accept::New);
        assert_eq!(evicted, Some(low_id));
        assert!(store.contains(&sos.id));
        assert!(!store.contains(&low_id));
    ...
    fn refuses_lower_priority_when_full_rather_than_evict_for_no_gain() {
        ...
        assert_eq!(store.accept(env(Priority::Sos, 5, 1000, 1), 0), (Accept::New, None));
        let low = env(Priority::Low, 5, 1000, 2);
        assert_eq!(store.accept(low, 0), (Accept::StoreFullLowerPriority, None));
        assert_eq!(store.len(), 1);
    }
    ...
    fn purge_expired_removes_only_expired() {
        let mut store = Store::new(10);
        let expiring = env(Priority::Normal, 5, 100, 1);
        let expiring_id = expiring.id;
        store.accept(expiring, 0);
        store.accept(env(Priority::Normal, 5, 9999, 2), 0);
        let purged = store.purge_expired(500);
        assert_eq!(purged, vec![expiring_id]);
        assert_eq!(store.len(), 1);
    }
